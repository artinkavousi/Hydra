...
const useRange = () => {
  return React.useCallback((v, size) => size * v, []);
};
const useInvertedRange = () => {
  return React.useCallback((v, size) => v / size, []);
};
...
const fadeIn = o => plugin.keyframes({
  '0%': {
    opacity: 0
  },
  '10%': {
...
const PreviewSvg = plugin.styled('svg', {
  width: '100%',
  overflow: 'visible',
  height: 6,
  '> circle': {
...
function BezierSvg({
  ...
  const handleLeft = React.useRef(null);
  const handleRight = React.useRef(null);
...
const DebouncedBezierPreview = React__default["default"].memo(({
  value
}) => {
  const [, forceUpdate] = React.useReducer(x => x + 1, 0);
  const plotPoints = Array(21).fill(0).map((_, i) => 5 + value.evaluate(i / 20) * 90);
  return React__default["default"].createElement(PreviewSvg, {
    onClick: forceUpdate
  }, plotPoints.map((p, i) => React__default["default"].createElement("circle", {
    key: i + Date.now(),
    r: 3,
    cx: `${p}%`,
    style: {
...
function BezierPreview({
  ...
  const [debouncedValue, set] = React.useState(value);
  const debounceValue = React.useMemo(() => plugin.debounce(v => set(v), 250), []);
  React.useEffect(() => void debounceValue(value), [value, debounceValue]);
  return React__default["default"].createElement(DebouncedBezierPreview, {
...
const normalize = (input = [0.25, 0.1, 0.25, 1]) => {
  ...
  const {
    value: _value,
    settings
  } = plugin.normalizeVector(handles, mergedSettings, ['x1', 'y1', 'x2', 'y2']);
  const value = _value;
...
const sanitize = (value, settings, prevValue) => {
  const _value = plugin.sanitizeVector(value, settings, prevValue);
...
function SelectBezier({
  ...
  const selectValue = React.useMemo(() => optionValues.find(v => v === value.toString()) || false, [value]);
  const args = {
    type: 'SELECT',
    value: selectValue,
  ...
  const select = plugin.useInputSetters(_objectSpread2(_objectSpread2({}, args), {}, {
    setValue
  }));
...
function Bezier() {
  ...
  return React__default["default"].createElement(React__default["default"].Fragment, null, React__default["default"].createElement(Row, {
    input: true
  ...
  }, "\uD835\uDC53"), React__default["default"].createElement(SelectBezier, {
    value: value,
    onUpdate: onUpdate
...
const bezier = plugin.createPlugin({
  normalize,
  sanitize,
  format: (value, settings) => plugin.formatVector(value, settings),
  component: Bezier
});
